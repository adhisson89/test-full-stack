...
export class AuthService {
  ...
  `;
    ...
    mutation CreateUser($username: String!, $password: String!, $role: String) {
      createUser(userRequest: { username: $username, password: $password, role: $role }) {
        id
        username
        role
      }
  ...
  `;
    ...
    private handleAuthentication(authResponse: AuthResponse): void {
        const expirationDate = new Date(new Date().getTime() + 3600 * 1000); // 1 hour expiration
    ...
    private fetchUserInfo(): void {
      ...
      if (token) {
        try {
          ...
          const user = {
            id: payload.userId, // Use userId from token instead of sub
            role: this.extractMainRole(payload.roles),
            username: payload.sub // Keep sub as username
          };
